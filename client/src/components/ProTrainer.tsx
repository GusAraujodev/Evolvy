...
import { Check, X, UserCircle2, ChevronRight, Loader2, Dumbbell } from 'lucide-react';
...
type ParsedWorkoutContent = {
  divisao: string;
  objetivo_resumo: string;
  semana: ParsedWorkoutDay[];
  observacoes: string;
};

function parseContent(content: any): ParsedWorkoutContent {
  ...
  const semana = Array.isArray(parsed?.semana)
    ? parsed.semana.map((day: any) => ({
        dia: String(day?.dia ?? day?.day ?? 'Dia'),
        tipo: String(day?.tipo ?? day?.type ?? ''),
        foco: String(day?.foco ?? day?.focus ?? ''),
        exercicios: Array.isArray(day?.exercicios)
          ? day.exercicios.map((exercise: any) => ({
              nome: String(exercise?.nome ?? exercise?.name ?? 'Exercício'),
              series: Number(exercise?.series ?? exercise?.sets ?? 0) || 0,
              repeticoes: String(exercise?.repeticoes ?? exercise?.reps ?? ''),
              descanso: String(exercise?.descanso ?? exercise?.rest ?? ''),
              tecnica: String(exercise?.tecnica ?? exercise?.technique ?? ''),
            }))
          : [],
      }))
    : [];
  ...
  return {
    divisao: String(parsed?.divisao ?? ''),
    objetivo_resumo: String(parsed?.objetivo_resumo ?? ''),
    semana,
...
export function ProTrainer() {
  ...
  const loadPlans = async () => {
    ...
    setPlans(plans as WorkoutPlanRow[]);
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground">
      <aside className="w-72 border-r border-border bg-card/40 backdrop-blur-sm p-4 flex flex-col gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Treino Pro</p>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {loading ? (
            ...
            <div className="rounded-2xl border border-border bg-background p-4 text-sm text-muted-foreground">
              Nenhum plano de treino para revisar.
            </div>
          ) : (
            plans.map((plan) => {
          ...
          <div className="h-full min-h-[70vh] flex items-center justify-center">
            <div className="text-center max-w-md">
              ...
              <p className="text-muted-foreground mt-3">
                Escolha um plano na lista ao lado para revisar a divisão, o objetivo, a semana de treino e registrar sua decisão.
              </p>
            </div>
          </div>
        ) : (
          <div className="max-w-5xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-3xl border border-border bg-card p-5">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Divisão</p>
                <p className="text-2xl font-black mt-3">{selectedWorkout.divisao || 'Sem divisão informada'}</p>
              </div>
              <div className="rounded-3xl border border-border bg-card p-5">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Objetivo</p>
                <p className="text-2xl font-black mt-3">{selectedWorkout.objetivo_resumo || 'Sem objetivo resumido'}</p>
              </div>
            </div>

            <div className="grid gap-4">
              {selectedWorkout.semana.length > 0 ? (
                selectedWorkout.semana.map((day, dayIndex) => (
                  <motion.div
                    key={dayIndex}
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                      <div>
                        <h3 className="text-xl font-black">{day.dia}</h3>
                    ...
                    {day.tipo.toLowerCase() === 'treino' && (
                      <div className="space-y-2">
                        {day.exercicios.length > 0 ? (
                          day.exercicios.map((exercise, exerciseIndex) => (
                            <div key={exerciseIndex} className="rounded-2xl bg-background border border-border px-4 py-3">
                              <div className="flex flex-wrap items-start justify-between gap-2">
                                ...
                                <div className="text-right text-sm text-muted-foreground">
                                  <p>{exercise.descanso}</p>
                                  {exercise.tecnica && <p>{exercise.tecnica}</p>}
                                </div>
                              </div>
                            </div>
                          ))
                        ) : (
                          <div className="rounded-2xl bg-background border border-border px-4 py-3 text-sm text-muted-foreground">
                            Nenhum exercício informado para este dia.
                          </div>
                        )}
                      </div>
                    )}
                ...
                <div className="rounded-3xl border border-border bg-card p-6 text-sm text-muted-foreground">
                  Este plano não contém uma semana de treino estruturada.
                </div>
              )}
            </div>

            {selectedWorkout.observacoes && (
              <div className="rounded-3xl border border-border bg-card p-6">
                <div className="flex items-center gap-2 mb-4">
                  <Dumbbell className="w-5 h-5 text-primary" />
                  <h3 className="text-xl font-black">Observações</h3>
                </div>
                <p className="text-sm text-muted-foreground whitespace-pre-line">{selectedWorkout.observacoes}</p>
              </div>
            )}
